...
|---|---|
| `zz_remake/V1`, `V2`, `V3` são especificação | São a **conversa** do remake, com propostas que se contradizem de propósito ao longo das versões. A conclusão está em `contexto_interno/` |
...
| `contexto_interno/12-visao-tecnologica.md` descreve o que está no ar | Descreve **para onde vamos**. O que está no ar é o `CLAUDE.md` |
| Existe uma pasta de arquivo histórico | Existia (`docs/`, `contexto_interno/90-arquivo/`) e foi **apagada em 2026-08-14**. O porquê ficou em `30-decisoes.md`; a narrativa, só no `git log` |
...
| O Agente 2 aceita uma ordem em linguagem natural | ⛔ **Não aceita.** A ação `refine_semantics` monta a entrada como `Definições atuais do usuário: {...}\nMelhore-as.` e não lê `prompt` nenhum. Havia um campo *"Ordem para o Agente 2"* no Editar Esquema que travava o botão quando vazio e era descartado — removido em 2026-09-03. ⭐ E não foi consertado mandando: o prompt dele diz *"PRESERVE O CONTEÚDO, você melhora a redação, não o conteúdo"*, porque a regra de negócio que a pessoa escreveu é a única fonte daquilo |
